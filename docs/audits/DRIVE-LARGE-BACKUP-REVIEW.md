...
## Terceira revisão — 55b1c24

Guarda pré-login corrigida, mas FAIL em corrida da inicialização. Reprodução Codex com cache pendente sintético, token válido, driveFileId=null e nenhum ID local: iniciar initApp mantendo GET de findDataFile em espera; disparar pageshow; então liberar GET que encontra arquivo existing. Observado GET, POST, PATCH: criou arquivo new apesar de existing já existir. appHydrated=true é ativado antes da busca/recuperação inicial completar.

Impedir retomada automática durante TODA inicialização (busca + sincronização inicial), liberando apenas quando concluída com estado seguro. Não basta hidratação síncrona. Redefinir prontidão ao iniciar e ao sair. Teste permanente deve atrasar descoberta, disparar pageshow/visible e provar zero POST e atualização somente do arquivo encontrado; preservar testes pré-login/hidratação. Atualizar relatório e rodar uma suíte completa. Reproduções ficam em scripts de auditoria externos, com dados sintéticos.
